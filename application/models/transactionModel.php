...
//        $q = $this->db->get($this->table);
    ...
    function get_front_table($limit = null)
    {
        ...
        $getData = $this->db->query("SELECT `transaction_id`,`deleteTransaction`, ABS(`cost`) as cst,`transaction`.`created_on`, ABS(`unit`) as unt, `detail`.`name` as sam, `product`.`product_name` as pam, `transaction`.`type` FROM `transaction` INNER JOIN `detail` ON `detail`.`type`=`transaction`.`type` INNER JOIN `product` ON `product`.`product_id`=`transaction`.`product_id` ORDER by transaction_id DESC LIMIT 10");
